...
print_help() {
    ...
    echo -e "Copy a selected emoji to the clipboard."
    echo -e ""
...
if (( VERBOSE )); then
    echo -e "${YELLOW}Killing any running wofi instances...${NC}"
fi
...
if (( VERBOSE )); then
    echo -e "${YELLOW}Selecting emoji...${NC}"
fi
...
if [[ -n "$selected_emoji" ]]; then
    if (( VERBOSE )); then
        echo -e "${YELLOW}Emoji selected: $selected_emoji${NC}"
    fi
    echo -n "$selected_emoji" | wl-copy
    notify-send "Emoji Copied!" \
    "The emoji $selected_emoji has been copied to the clipboard."
else
    if (( VERBOSE )); then
        echo -e "${YELLOW}No emoji selected.${NC}"
    fi
    notify-send "No Emoji Selected" "You didn't select an emoji."
fi
exit
